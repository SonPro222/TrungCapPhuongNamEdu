...
@RestController
@RequestMapping(ThiPath.LICH_THI)
@RequiredArgsConstructor
public class LichThiController {
    private final LichThiService service;

    @GetMapping
    public ResponseEntity<ApiResponse<Page<LichThiResponse>>> findAll(Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.ok(service.findAll(pageable)));
    }

    @GetMapping(ThiPath.ID)
    public ResponseEntity<ApiResponse<LichThiResponse>> findById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.ok(service.findById(id)));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<LichThiResponse>> create(@Valid @RequestBody LichThiRequest request) {
        return ResponseEntity.status(201).body(ApiResponse.created(service.create(request)));
    }

    @PutMapping(ThiPath.ID)
    public ResponseEntity<ApiResponse<LichThiResponse>> update(@PathVariable Long id, @Valid @RequestBody LichThiRequest request) {

...
import { getAllLichThi } from '@/api/thi/response/fetchResponse_LichThi.js';
import { createLichThi, updateLichThi, deleteLichThi } from '@/api/thi/request/fetchRequest_LichThi.js';

const pageTitle = 'Lịch thi';
const fields = [
  {
    "key": "lopHocPhanId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLichThi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLichThi(editingId.value, payload);
    } else {

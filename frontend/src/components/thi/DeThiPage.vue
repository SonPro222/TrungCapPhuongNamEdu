...
import { getAllDeThi } from '@/api/thi/response/fetchResponse_DeThi.js';
import { createDeThi, updateDeThi, deleteDeThi } from '@/api/thi/request/fetchRequest_DeThi.js';

const pageTitle = 'Đề thi';
const fields = [
  {
    "key": "lichThiId",
    "label": "Lich thi id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDeThi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateDeThi(editingId.value, payload);
    } else {

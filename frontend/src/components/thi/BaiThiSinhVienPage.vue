...
import { getAllBaiThiSinhVien } from '@/api/thi/response/fetchResponse_BaiThiSinhVien.js';
import { createBaiThiSinhVien, updateBaiThiSinhVien, deleteBaiThiSinhVien } from '@/api/thi/request/fetchRequest_BaiThiSinhVien.js';

const pageTitle = 'Bài thi sinh viên';
const fields = [
  {
    "key": "deThiId",
    "label": "De thi id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllBaiThiSinhVien();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateBaiThiSinhVien(editingId.value, payload);
    } else {
      await createBaiThiSinhVien(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

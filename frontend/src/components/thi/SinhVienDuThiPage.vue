...
import { getAllSinhVienDuThi } from '@/api/thi/response/fetchResponse_SinhVienDuThi.js';
import { createSinhVienDuThi, updateSinhVienDuThi, deleteSinhVienDuThi } from '@/api/thi/request/fetchRequest_SinhVienDuThi.js';

const pageTitle = 'Sinh viên dự thi';
const fields = [
  {
    "key": "lichThiId",
    "label": "Lich thi id",
  ...
  {
    "key": "duDieuKienThi",
    "label": "Du dieu kien thi",
    "type": "checkbox"
  },
  {
    "key": "lyDoKhongDuDieuKien",
    "label": "Ly do khong du dieu kien",
    "type": "checkbox"
  },
  {
    "key": "trangThai",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllSinhVienDuThi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateSinhVienDuThi(editingId.value, payload);
    } else {
      await createSinhVienDuThi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

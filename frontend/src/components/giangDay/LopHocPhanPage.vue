...
import { getAllLopHocPhan } from '@/api/giangDay/response/fetchResponse_LopHocPhan.js';
import { createLopHocPhan, updateLopHocPhan, deleteLopHocPhan } from '@/api/giangDay/request/fetchRequest_LopHocPhan.js';

const pageTitle = 'Lớp học phần';
const fields = [
  {
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLopHocPhan();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLopHocPhan(editingId.value, payload);
    } else {
      await createLopHocPhan(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

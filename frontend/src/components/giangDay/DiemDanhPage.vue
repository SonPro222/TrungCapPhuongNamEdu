...
import { getAllDiemDanh } from '@/api/giangDay/response/fetchResponse_DiemDanh.js';
import { createDiemDanh, updateDiemDanh, deleteDiemDanh } from '@/api/giangDay/request/fetchRequest_DiemDanh.js';

const pageTitle = 'Điểm danh';
const fields = [
  {
    "key": "lichHocId",
    "label": "Lich hoc id",
  ...
  {
    "key": "thoiGianDiemDanh",
    "label": "Thoi gian diem danh",
    "type": "datetime-local"
  }
];
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDiemDanh();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateDiemDanh(editingId.value, payload);
    } else {
      await createDiemDanh(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

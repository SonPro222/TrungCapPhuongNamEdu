...
import { getAllSinhVienLopHocPhan } from '@/api/giangDay/response/fetchResponse_SinhVienLopHocPhan.js';
import { createSinhVienLopHocPhan, updateSinhVienLopHocPhan, deleteSinhVienLopHocPhan } from '@/api/giangDay/request/fetchRequest_SinhVienLopHocPhan.js';

const pageTitle = 'Sinh viên lớp học phần';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllSinhVienLopHocPhan();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateSinhVienLopHocPhan(editingId.value, payload);
    } else {
      await createSinhVienLopHocPhan(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteSinhVienLopHocPhan(id);
    await loadData();
  } catch (err) {

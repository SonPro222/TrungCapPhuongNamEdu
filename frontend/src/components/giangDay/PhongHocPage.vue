...
import { getAllPhongHoc } from '@/api/giangDay/response/fetchResponse_PhongHoc.js';
import { createPhongHoc, updatePhongHoc, deletePhongHoc } from '@/api/giangDay/request/fetchRequest_PhongHoc.js';

const pageTitle = 'Phòng học';
const fields = [
  {
    "key": "maPhong",
    "label": "Ma phong",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllPhongHoc();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updatePhongHoc(editingId.value, payload);
    } else {
      await createPhongHoc(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { createGiaoVien, updateGiaoVien, deleteGiaoVien } from '@/api/giangDay/request/fetchRequest_GiaoVien.js';

const pageTitle = 'Giáo viên';
const fields = [
  {
    "key": "maGiaoVien",
    "label": "Ma giao vien",
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateGiaoVien(editingId.value, payload);
    } else {
      await createGiaoVien(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllYeuCauDoiLich } from '@/api/giangDay/response/fetchResponse_YeuCauDoiLich.js';
import { createYeuCauDoiLich, updateYeuCauDoiLich, deleteYeuCauDoiLich } from '@/api/giangDay/request/fetchRequest_YeuCauDoiLich.js';

const pageTitle = 'Yêu cầu đổi lịch';
const fields = [
  {
    "key": "lichHocCuId",
    "label": "Lich hoc cu id",
    "type": "number"
  },
  {
    "key": "giaoVienId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllYeuCauDoiLich();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateYeuCauDoiLich(editingId.value, payload);
    } else {
      await createYeuCauDoiLich(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllCaHoc } from '@/api/giangDay/response/fetchResponse_CaHoc.js';
import { createCaHoc, updateCaHoc, deleteCaHoc } from '@/api/giangDay/request/fetchRequest_CaHoc.js';

const pageTitle = 'Ca học';
const fields = [
  {
    "key": "maCa",
    "label": "Ma ca",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllCaHoc();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateCaHoc(editingId.value, payload);
    } else {
      await createCaHoc(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

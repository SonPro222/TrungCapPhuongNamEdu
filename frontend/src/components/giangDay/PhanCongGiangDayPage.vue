...
import { getAllPhanCongGiangDay } from '@/api/giangDay/response/fetchResponse_PhanCongGiangDay.js';
import { createPhanCongGiangDay, updatePhanCongGiangDay, deletePhanCongGiangDay } from '@/api/giangDay/request/fetchRequest_PhanCongGiangDay.js';

const pageTitle = 'Phân công giảng dạy';
const fields = [
  {
    "key": "lopHocPhanId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllPhanCongGiangDay();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updatePhanCongGiangDay(editingId.value, payload);
    } else {
      await createPhanCongGiangDay(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deletePhanCongGiangDay(id);
    await loadData();
  } catch (err) {

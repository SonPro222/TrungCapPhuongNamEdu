...
import { getAllLichHoc } from '@/api/giangDay/response/fetchResponse_LichHoc.js';
import { createLichHoc, updateLichHoc, deleteLichHoc } from '@/api/giangDay/request/fetchRequest_LichHoc.js';

const pageTitle = 'Lịch học';
const fields = [
  {
    "key": "lopHocPhanId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLichHoc();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLichHoc(editingId.value, payload);
    } else {
      await createLichHoc(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllLichSuChinhDiemBaiTap } from '@/api/lms/response/fetchResponse_LichSuChinhDiemBaiTap.js';
import { createLichSuChinhDiemBaiTap, updateLichSuChinhDiemBaiTap, deleteLichSuChinhDiemBaiTap } from '@/api/lms/request/fetchRequest_LichSuChinhDiemBaiTap.js';

const pageTitle = 'Lịch sử chỉnh điểm bài tập';
const fields = [
  {
    "key": "ketQuaBaiTapLopId",
    "label": "Ket qua bai tap lop id",
    "type": "number"
  },
  {
    "key": "nguoiChinhId",
    "label": "Nguoi chinh id",
    "type": "number"
  },
  {
    "key": "vaiTroChinh",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLichSuChinhDiemBaiTap();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLichSuChinhDiemBaiTap(editingId.value, payload);
    } else {
      await createLichSuChinhDiemBaiTap(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteLichSuChinhDiemBaiTap(id);
    await loadData();
  } catch (err) {

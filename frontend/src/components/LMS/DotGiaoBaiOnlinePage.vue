...
import { getAllDotGiaoBaiOnline } from '@/api/lms/response/fetchResponse_DotGiaoBaiOnline.js';
import { createDotGiaoBaiOnline, updateDotGiaoBaiOnline, deleteDotGiaoBaiOnline } from '@/api/lms/request/fetchRequest_DotGiaoBaiOnline.js';

const pageTitle = 'Đợt giao bài online';
const fields = [
  {
    "key": "baiTapOnlineId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDotGiaoBaiOnline();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateDotGiaoBaiOnline(editingId.value, payload);
    } else {
      await createDotGiaoBaiOnline(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteDotGiaoBaiOnline(id);
    await loadData();
  } catch (err) {

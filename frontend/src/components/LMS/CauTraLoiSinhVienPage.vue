...
import { getAllCauTraLoiSinhVien } from '@/api/lms/response/fetchResponse_CauTraLoiSinhVien.js';
import { createCauTraLoiSinhVien, updateCauTraLoiSinhVien, deleteCauTraLoiSinhVien } from '@/api/lms/request/fetchRequest_CauTraLoiSinhVien.js';

const pageTitle = 'Câu trả lời sinh viên';
const fields = [
  {
    "key": "lanLamBaiId",
    "label": "Lan lam bai id",
    "type": "checkbox"
  },
  {
    "key": "cauHoiId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllCauTraLoiSinhVien();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateCauTraLoiSinhVien(editingId.value, payload);
    } else {
      await createCauTraLoiSinhVien(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteCauTraLoiSinhVien(id);
    await loadData();
  } catch (err) {

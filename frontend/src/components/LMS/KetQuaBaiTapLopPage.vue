...
import { getAllKetQuaBaiTapLop } from '@/api/lms/response/fetchResponse_KetQuaBaiTapLop.js';
import { createKetQuaBaiTapLop, updateKetQuaBaiTapLop, deleteKetQuaBaiTapLop } from '@/api/lms/request/fetchRequest_KetQuaBaiTapLop.js';

const pageTitle = 'Kết quả bài tập lớp';
const fields = [
  {
    "key": "baiTapLopId",
    "label": "Bai tap lop id",
    "type": "number"
  },
  {
    "key": "sinhVienId",
  ...
  {
    "key": "lanLamDuocChonId",
    "label": "Lan lam duoc chon id",
    "type": "checkbox"
  },
  {
    "key": "diemHeThong",
  ...
  {
    "key": "diemGiaoVienChinhSua",
    "label": "Diem giao vien chinh sua",
    "type": "number"
  },
  {
    "key": "diemCuoiCung",
    "label": "Diem cuoi cung",
    "type": "number"
  },
  {
    "key": "ketQua",
  ...
  {
    "key": "ngayDaySangDiem",
    "label": "Ngay day sang diem",
    "type": "date"
  },
  {
    "key": "nguoiDaySangDiemId",
    "label": "Nguoi day sang diem id",
    "type": "number"
  }
];
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllKetQuaBaiTapLop();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateKetQuaBaiTapLop(editingId.value, payload);
    } else {
      await createKetQuaBaiTapLop(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteKetQuaBaiTapLop(id);
    await loadData();
  } catch (err) {

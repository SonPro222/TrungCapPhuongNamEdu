...
import { getAllLichSuBaiTapLop } from '@/api/lms/response/fetchResponse_LichSuBaiTapLop.js';
import { createLichSuBaiTapLop, updateLichSuBaiTapLop, deleteLichSuBaiTapLop } from '@/api/lms/request/fetchRequest_LichSuBaiTapLop.js';

const pageTitle = 'Lịch sử bài tập lớp';
const fields = [
  {
    "key": "baiTapLopId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLichSuBaiTapLop();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLichSuBaiTapLop(editingId.value, payload);
    } else {
      await createLichSuBaiTapLop(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteLichSuBaiTapLop(id);
    await loadData();
  } catch (err) {

...
import { getAllBaiTapOnline } from '@/api/lms/response/fetchResponse_BaiTapOnline.js';
import { createBaiTapOnline, updateBaiTapOnline, deleteBaiTapOnline } from '@/api/lms/request/fetchRequest_BaiTapOnline.js';

const pageTitle = 'Bài tập online';
const fields = [
  {
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllBaiTapOnline();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateBaiTapOnline(editingId.value, payload);
    } else {
      await createBaiTapOnline(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

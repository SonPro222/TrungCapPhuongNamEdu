...
import { getAllLichSuDayDiemLms } from '@/api/lms/response/fetchResponse_LichSuDayDiemLms.js';
import { createLichSuDayDiemLms, updateLichSuDayDiemLms, deleteLichSuDayDiemLms } from '@/api/lms/request/fetchRequest_LichSuDayDiemLms.js';

const pageTitle = 'Lịch sử đẩy điểm LMS';
const fields = [
  {
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLichSuDayDiemLms();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLichSuDayDiemLms(editingId.value, payload);
    } else {
      await createLichSuDayDiemLms(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteLichSuDayDiemLms(id);
    await loadData();
  } catch (err) {

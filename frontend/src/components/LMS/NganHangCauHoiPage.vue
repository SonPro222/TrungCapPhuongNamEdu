...
import { getAllNganHangCauHoi } from '@/api/lms/response/fetchResponse_NganHangCauHoi.js';
import { createNganHangCauHoi, updateNganHangCauHoi, deleteNganHangCauHoi } from '@/api/lms/request/fetchRequest_NganHangCauHoi.js';

const pageTitle = 'Ngân hàng câu hỏi';
const fields = [
  {
    "key": "chuongTrinhMonId",
    "label": "Chuong trinh mon id",
    "type": "number"
  },
  {
    "key": "nguoiTaoTaiKhoanId",
    "label": "Nguoi tao tai khoan id",
    "type": "number"
  },
  {
    "key": "nguonTao",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllNganHangCauHoi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateNganHangCauHoi(editingId.value, payload);
    } else {
      await createNganHangCauHoi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllLanLamBaiOnline } from '@/api/lms/response/fetchResponse_LanLamBaiOnline.js';
import { createLanLamBaiOnline, updateLanLamBaiOnline, deleteLanLamBaiOnline } from '@/api/lms/request/fetchRequest_LanLamBaiOnline.js';

const pageTitle = 'Lần làm bài online';
const fields = [
  {
    "key": "baiTapLopId",
  ...
  {
    "key": "diemGiaoVienCham",
    "label": "Diem giao vien cham",
    "type": "number"
  },
  {
    "key": "diemCuoiCung",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllLanLamBaiOnline();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateLanLamBaiOnline(editingId.value, payload);
    } else {
      await createLanLamBaiOnline(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteLanLamBaiOnline(id);
    await loadData();
  } catch (err) {

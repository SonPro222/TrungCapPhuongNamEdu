...
import { getAllBaiTapOnlineCauHoi } from '@/api/lms/response/fetchResponse_BaiTapOnlineCauHoi.js';
import { createBaiTapOnlineCauHoi, updateBaiTapOnlineCauHoi, deleteBaiTapOnlineCauHoi } from '@/api/lms/request/fetchRequest_BaiTapOnlineCauHoi.js';

const pageTitle = 'Bài tập online câu hỏi';
const fields = [
  {
    "key": "baiTapOnlineId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllBaiTapOnlineCauHoi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateBaiTapOnlineCauHoi(editingId.value, payload);
    } else {
      await createBaiTapOnlineCauHoi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteBaiTapOnlineCauHoi(id);
    await loadData();
  } catch (err) {

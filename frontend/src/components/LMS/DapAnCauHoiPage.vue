...
import { getAllDapAnCauHoi } from '@/api/lms/response/fetchResponse_DapAnCauHoi.js';
import { createDapAnCauHoi, updateDapAnCauHoi, deleteDapAnCauHoi } from '@/api/lms/request/fetchRequest_DapAnCauHoi.js';

const pageTitle = 'Đáp án câu hỏi';
const fields = [
  {
    "key": "cauHoiId",
    "label": "Cau hoi id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDapAnCauHoi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateDapAnCauHoi(editingId.value, payload);
    } else {
      await createDapAnCauHoi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Mục tiêu chương trình</h1>
        <p>Quản lý dữ liệu bảng muc_tieu_chuong_trinh theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} mục tiêu chương trình</h2>
        ...
        <div class="form-grid">
          ...
          <label class="field">
            ...
            <select v-model="form.loai">
              ...
              <option value="kien_thuc">kien_thuc</option>
              <option value="ky_nang">ky_nang</option>
              <option value="nang_luc_tu_chu_trach_nhiem">nang_luc_tu_chu_trach_nhiem</option>
            </select>
          </label>
    ...
    <div class="card">
      <div class="table-toolbar">
        <h2>Danh sách mục tiêu chương trình</h2>
...
import { createMucTieuChuongTrinh, updateMucTieuChuongTrinh, deleteMucTieuChuongTrinh } from '@/api/chuongTrinh/ApiRequest/MucTieuChuongTrinhController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllMucTieuChuongTrinh();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    ...
    const matchedKeyword = !keywordValue || [item.maMucTieu, item.noiDung, item.ghiChu]
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateMucTieuChuongTrinh(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createMucTieuChuongTrinh(payload);
...
const removeItem = async (id) => {
  ...
  success.value = '';
  try {
    await deleteMucTieuChuongTrinh(id);

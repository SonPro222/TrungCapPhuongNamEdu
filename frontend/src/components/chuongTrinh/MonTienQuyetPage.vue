<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Môn tiên quyết</h1>
        <p>Quản lý dữ liệu bảng mon_tien_quyet theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} môn tiên quyết</h2>
      <form @submit.prevent="saveData">
        <div class="form-grid">
          <label class="field">
            <span>Môn ID</span>
            <input v-model="form.monId" type="number" step="0.01" placeholder="Nhập môn id" />
          </label>
          <label class="field">
            <span>Môn điều kiện ID</span>
            <input v-model="form.monDieuKienId" type="number" step="0.01" placeholder="Nhập môn điều kiện id" />
          </label>
          <label class="field">
            <span>Loại</span>
            <select v-model="form.loai">
              <option value="">-- Chọn --</option>
              <option value="tien_quyet">tien_quyet</option>
              <option value="song_hanh">song_hanh</option>
              <option value="hoc_truoc">hoc_truoc</option>
            </select>
          </label>
    ...
    <div class="card">
      <div class="table-toolbar">
        <h2>Danh sách môn tiên quyết</h2>
      ...
      <div class="table-wrap">
        <table>
          ...
          <tr v-for="item in filteredItems" :key="item.id">
            ...
            <td>{{ formatValue(item.monId) }}</td>
            <td>{{ formatValue(item.monDieuKienId) }}</td>
...
import { createMonTienQuyet, updateMonTienQuyet, deleteMonTienQuyet } from '@/api/chuongTrinh/ApiRequest/MonTienQuyetController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllMonTienQuyet();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    ...
    const matchedKeyword = !keywordValue || [item.loaiDieuKien, item.ghiChu]
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateMonTienQuyet(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createMonTienQuyet(payload);

<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Vị trí việc làm</h1>
        <p>Quản lý dữ liệu bảng vi_tri_viec_lam theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} vị trí việc làm</h2>
      <form @submit.prevent="saveData">
        <div class="form-grid">
          ...
          <label class="field">
            ...
            <input v-model="form.ten" type="text" placeholder="Nhập tên vị trí" />
          </label>
    ...
    <div class="card">
      <div class="table-toolbar">
        <h2>Danh sách vị trí việc làm</h2>
...
import { createViTriViecLam, updateViTriViecLam, deleteViTriViecLam } from '@/api/chuongTrinh/ApiRequest/ViTriViecLamController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllViTriViecLam();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    ...
    const matchedKeyword = !keywordValue || [item.tenViTri, item.moTa, item.ghiChu]
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateViTriViecLam(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createViTriViecLam(payload);

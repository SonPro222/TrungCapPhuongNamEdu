<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Môn học</h1>
        <p>Quản lý dữ liệu bảng mon_hoc theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} môn học</h2>
      <form @submit.prevent="saveData">
        <div class="form-grid">
          <label class="field">
            <span>Mã môn</span>
            <input v-model="form.maMon" type="text" placeholder="Nhập mã môn" />
          </label>
          <label class="field">
            <span>Tên môn</span>
            <input v-model="form.tenMon" type="text" placeholder="Nhập tên môn" />
          </label>
          <label class="field field-full">
            <span>Mô tả</span>
            <textarea v-model="form.moTa" rows="3" placeholder="Nhập mô tả"></textarea>
          </label>
        </div>
        <div class="actions">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tr v-for="item in filteredItems" :key="item.id">
            ...
            <td>{{ formatValue(item.maMon) }}</td>
            <td>{{ formatValue(item.tenMon) }}</td>
            <td>{{ formatValue(item.moTa) }}</td>
...
import { createMonHoc, updateMonHoc, deleteMonHoc } from '@/api/chuongTrinh/ApiRequest/MonHocController.js';
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => String(item.maMon || '').toLowerCase().includes(keyword) || String(item.tenMon || '').toLowerCase().includes(keyword) || String(item.moTa || '').toLowerCase().includes(keyword));
});
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateMonHoc(selectedId.value, payload);

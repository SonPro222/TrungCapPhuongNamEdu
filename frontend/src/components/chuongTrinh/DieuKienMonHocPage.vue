<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Điều kiện môn học</h1>
        <p>Quản lý dữ liệu bảng dieu_kien_mon_hoc theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} điều kiện môn học</h2>
        ...
        <div class="form-grid">
          ...
          <label class="field">
            ...
            <select v-model="form.loai">
              ...
              <option value="phong_hoc">phong_hoc</option>
              <option value="thiet_bi">thiet_bi</option>
              <option value="hoc_lieu">hoc_lieu</option>
              <option value="dung_cu">dung_cu</option>
              <option value="nguyen_vat_lieu">nguyen_vat_lieu</option>
    ...
    <div class="card">
      <div class="table-toolbar">
        <h2>Danh sách điều kiện môn học</h2>
...
import { createDieuKienMonHoc, updateDieuKienMonHoc, deleteDieuKienMonHoc } from '@/api/chuongTrinh/ApiRequest/DieuKienMonHocController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDieuKienMonHoc();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    ...
    const matchedKeyword = !keywordValue || [item.loaiDieuKien, item.noiDung, item.ghiChu]
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateDieuKienMonHoc(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createDieuKienMonHoc(payload);

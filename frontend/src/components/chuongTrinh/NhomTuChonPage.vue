<template>
  <section class="page">
    <div class="page-header">
      <div>
        <h1>Nhóm tự chọn</h1>
        <p>Quản lý dữ liệu bảng nhom_tu_chon theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} nhóm tự chọn</h2>
      <form @submit.prevent="saveData">
        <div class="form-grid">
          ...
          <label class="field">
            ...
            <input v-model="form.soMonChon" type="number" step="0.01" placeholder="Nhập số môn chọn" />
          </label>
          <label class="field">
            <span>Số tín chỉ cần đạt</span>
            <input v-model="form.soTinChiCanDat" type="number" step="0.01" placeholder="Nhập số tín chỉ cần đạt" />
          </label>
      ...
      <div class="table-wrap">
        <table>
          ...
          <tr v-for="item in filteredItems" :key="item.id">
            ...
            <td>{{ formatValue(item.soMonChon) }}</td>
            <td>{{ formatValue(item.soTinChiCanDat) }}</td>
...
import { getAllNhomTuChon } from '@/api/chuongTrinh/ApiRespone/NhomTuChonController.js';
import { createNhomTuChon, updateNhomTuChon, deleteNhomTuChon } from '@/api/chuongTrinh/ApiRequest/NhomTuChonController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllNhomTuChon();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => String(item.ten || '').toLowerCase().includes(keyword) || String(item.ghiChu || '').toLowerCase().includes(keyword));
});
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateNhomTuChon(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createNhomTuChon(payload);

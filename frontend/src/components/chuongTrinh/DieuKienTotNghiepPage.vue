  ...
  <section class="page">
    <div class="page-header">
      <div>
        ...
        <p>Quản lý dữ liệu bảng dieu_kien_tot_nghiep theo chương trình đào tạo.</p>
      </div>
    ...
    <div class="card">
      <h2>{{ isEdit ? 'Cập nhật' : 'Thêm mới' }} điều kiện tốt nghiệp</h2>
    ...
    <div class="card">
      <div class="table-toolbar">
        <h2>Danh sách điều kiện tốt nghiệp</h2>
...
import { createDieuKienTotNghiep, updateDieuKienTotNghiep, deleteDieuKienTotNghiep } from '@/api/chuongTrinh/ApiRequest/DieuKienTotNghiepController.js';
...
const fetchData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDieuKienTotNghiep();
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => String(item.noiDung || '').toLowerCase().includes(keyword));
});
...
const saveData = async () => {
  ...
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await updateDieuKienTotNghiep(selectedId.value, payload);
      success.value = 'Cập nhật thành công';
    } else {
      await createDieuKienTotNghiep(payload);
...
const removeItem = async (id) => {
  ...
  success.value = '';
  try {
    await deleteDieuKienTotNghiep(id);

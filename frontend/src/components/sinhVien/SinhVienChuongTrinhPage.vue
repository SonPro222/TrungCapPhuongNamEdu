...
import { createSinhVienChuongTrinh, updateSinhVienChuongTrinh, deleteSinhVienChuongTrinh } from '@/api/sinhVien/request/fetchRequest_SinhVienChuongTrinh.js';

const pageTitle = 'Sinh viên chương trình';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllSinhVienChuongTrinh();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateSinhVienChuongTrinh(editingId.value, payload);
    } else {
      await createSinhVienChuongTrinh(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteSinhVienChuongTrinh(id);
    await loadData();
  } catch (err) {

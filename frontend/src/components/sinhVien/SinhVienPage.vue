...
import { getAllSinhVien } from '@/api/sinhVien/response/fetchResponse_SinhVien.js';
import { createSinhVien, updateSinhVien, deleteSinhVien } from '@/api/sinhVien/request/fetchRequest_SinhVien.js';

const pageTitle = 'Sinh viên';
const fields = [
  {
    "key": "maSinhVien",
    "label": "Ma sinh vien",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllSinhVien();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateSinhVien(editingId.value, payload);
    } else {

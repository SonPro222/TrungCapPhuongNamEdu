...
import { getAllMienTruBaoLuu } from '@/api/sinhVien/response/fetchResponse_MienTruBaoLuu.js';
import { createMienTruBaoLuu, updateMienTruBaoLuu, deleteMienTruBaoLuu } from '@/api/sinhVien/request/fetchRequest_MienTruBaoLuu.js';

const pageTitle = 'Miễn trừ bảo lưu';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllMienTruBaoLuu();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateMienTruBaoLuu(editingId.value, payload);
    } else {
      await createMienTruBaoLuu(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

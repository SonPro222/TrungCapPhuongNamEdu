...
import { getAllGiaoDichHocPhi } from '@/api/hocPhi/response/fetchResponse_GiaoDichHocPhi.js';
import { createGiaoDichHocPhi, updateGiaoDichHocPhi, deleteGiaoDichHocPhi } from '@/api/hocPhi/request/fetchRequest_GiaoDichHocPhi.js';

const pageTitle = 'Giao dịch học phí';
const fields = [
  {
    "key": "hocPhiId",
    "label": "Hoc phi id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllGiaoDichHocPhi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateGiaoDichHocPhi(editingId.value, payload);
    } else {
      await createGiaoDichHocPhi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

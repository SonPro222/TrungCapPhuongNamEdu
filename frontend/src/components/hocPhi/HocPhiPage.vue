...
import { getAllHocPhi } from '@/api/hocPhi/response/fetchResponse_HocPhi.js';
import { createHocPhi, updateHocPhi, deleteHocPhi } from '@/api/hocPhi/request/fetchRequest_HocPhi.js';

const pageTitle = 'Học phí';
const fields = [
  {
    "key": "sinhVienChuongTrinhId",
    "label": "Sinh vien chuong trinh id",
    "type": "number"
  },
  {
    "key": "tenKhoanThu",
  ...
  {
    "key": "soTienPhaiDong",
    "label": "So tien phai dong",
    "type": "number"
  },
  {
    "key": "soTienDaDong",
    "label": "So tien da dong",
    "type": "number"
  },
  {
    "key": "hanDong",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllHocPhi();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateHocPhi(editingId.value, payload);
    } else {
      await createHocPhi(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllKetQuaMonHoc } from '@/api/diem/response/fetchResponse_KetQuaMonHoc.js';
import { createKetQuaMonHoc, updateKetQuaMonHoc, deleteKetQuaMonHoc } from '@/api/diem/request/fetchRequest_KetQuaMonHoc.js';

const pageTitle = 'Kết quả môn học';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllKetQuaMonHoc();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateKetQuaMonHoc(editingId.value, payload);
    } else {
      await createKetQuaMonHoc(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllBaiKiemTra } from '@/api/diem/response/fetchResponse_BaiKiemTra.js';
import { createBaiKiemTra, updateBaiKiemTra, deleteBaiKiemTra } from '@/api/diem/request/fetchRequest_BaiKiemTra.js';

const pageTitle = 'Bài kiểm tra';
const fields = [
  {
    "key": "lopHocPhanId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllBaiKiemTra();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateBaiKiemTra(editingId.value, payload);
    } else {
      await createBaiKiemTra(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

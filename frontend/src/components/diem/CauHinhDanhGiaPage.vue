...
import { getAllCauHinhDanhGia } from '@/api/diem/response/fetchResponse_CauHinhDanhGia.js';
import { createCauHinhDanhGia, updateCauHinhDanhGia, deleteCauHinhDanhGia } from '@/api/diem/request/fetchRequest_CauHinhDanhGia.js';

const pageTitle = 'Cấu hình đánh giá';
const fields = [
  {
    "key": "lopHocPhanId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllCauHinhDanhGia();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateCauHinhDanhGia(editingId.value, payload);
    } else {
      await createCauHinhDanhGia(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllDiemChiTiet } from '@/api/diem/response/fetchResponse_DiemChiTiet.js';
import { createDiemChiTiet, updateDiemChiTiet, deleteDiemChiTiet } from '@/api/diem/request/fetchRequest_DiemChiTiet.js';

const pageTitle = 'Điểm chi tiết';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllDiemChiTiet();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateDiemChiTiet(editingId.value, payload);
    } else {
      await createDiemChiTiet(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

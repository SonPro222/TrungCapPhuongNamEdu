...
import { getAllBaiNopSinhVien } from '@/api/diem/response/fetchResponse_BaiNopSinhVien.js';
import { createBaiNopSinhVien, updateBaiNopSinhVien, deleteBaiNopSinhVien } from '@/api/diem/request/fetchRequest_BaiNopSinhVien.js';

const pageTitle = 'Bài nộp sinh viên';
const fields = [
  {
    "key": "baiKiemTraId",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllBaiNopSinhVien();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateBaiNopSinhVien(editingId.value, payload);
    } else {
      await createBaiNopSinhVien(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {

...
import { getAllKetQuaLopHocPhan } from '@/api/diem/response/fetchResponse_KetQuaLopHocPhan.js';
import { createKetQuaLopHocPhan, updateKetQuaLopHocPhan, deleteKetQuaLopHocPhan } from '@/api/diem/request/fetchRequest_KetQuaLopHocPhan.js';

const pageTitle = 'Kết quả lớp học phần';
const fields = [
  {
    "key": "sinhVienId",
    "label": "Sinh vien id",
  ...
  {
    "key": "diemChuyenCan",
    "label": "Diem chuyen can",
    "type": "number"
  },
  {
    "key": "diemQuaTrinh",
    "label": "Diem qua trinh",
    "type": "number"
  },
  {
    "key": "diemThi",
...
const loadData = async () => {
  ...
  error.value = '';
  try {
    const response = await getAllKetQuaLopHocPhan();
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await updateKetQuaLopHocPhan(editingId.value, payload);
    } else {
      await createKetQuaLopHocPhan(payload);
    }
    resetForm();
    await loadData();
  } catch (err) {
...
const removeItem = async (id) => {
  ...
  error.value = '';
  try {
    await deleteKetQuaLopHocPhan(id);
    await loadData();
  } catch (err) {

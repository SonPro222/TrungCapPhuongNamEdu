<template>
  <div class="tang-page">
    <XemTomTatHuongDi tieu-de="Tầng 4 - Kỳ" :danh-sach="summary" />

    <h2 class="tang-title">Kỳ trong version chương trình</h2>

    <XemTrangDangTai v-if="loading" />
    <p v-else-if="loi" class="loi-text">{{ loi }}</p>
    <XemTrangRong v-else-if="!danhSach.length" thong-bao="Version này chưa có kỳ đã gán." />
    <XemDanhSachNut
      v-else
      :danh-sach="danhSach"
      :ten-keys="['tenKy', 'maKy', 'tenKhungKyGoc']"
      :mo-ta-keys="['maKy']"
      @chon="moNhomKienThuc"
    />
  </div>
...
async function taiDuLieu() {
  ...
  loi.value = ''
  try {
    const [nganh, chuongTrinh, version, items] = await Promise.all([
      ...
      xemChuongTrinhService.layKyTheoVersion(versionId.value)
    ])
...
function moNhomKienThuc(item) {
  router.push({
    name: 'XemChuongTrinh.Tang5NhomKienThuc',
    params: { ...route.params, khungKyId: item.id }
  })
}

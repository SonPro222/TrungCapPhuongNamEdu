<template>
  <div class="dao-tao-mon-hoc-mau">
    ...
    <section class="khoi-dau">
      <div>
        <h2>Quản lý môn mẫu</h2>
        <p>
          CRUD danh mục môn học dùng chung toàn hệ thống. Môn mẫu không thuộc ngành, không thuộc chương trình, không thuộc version.
        </p>
      </div>
    ...
    <section class="luoi-chinh">
      <div class="the">
        <div class="tieu-de-the">
          <div>
            <h3>{{ formMon.id ? 'Cập nhật môn mẫu' : 'Thêm môn mẫu' }}</h3>
      ...
      <div class="the">
        <div class="tieu-de-the">
          <div>
            <h3>Danh sách môn mẫu</h3>
            <p>Tổng số môn đang tải: {{ danhSachMonHocmauLoc.length }}</p>
          </div>
        </div>
        ...
        <div class="bang-boc">
          <table>
            ...
            <tr v-if="!danhSachMonHocmauLoc.length">
              <td colspan="4" class="rong">Chưa có môn mẫu phù hợp.</td>
            </tr>

            <tr
                v-for="mon in danhSachMonHocmauLoc"
                :key="mon.id"
...
const danhSachMonHocmau = computed(() => {
  return [...danhSachMonHocRaw.value]
...
const danhSachMonHocmauLoc = computed(() => {
  ...
  return danhSachMonHocmau.value.filter((mon) => {
    const text = [
      mon.maMon,
      mon.ma,
      mon.tenMon,
...
async function taiDuLieu({ hienThongBao = true } = {}) {
  dangTai.value = true

  try {
    const result = await services.monHoc.getAll({ size: 1000 })
    danhSachMonHocRaw.value = result.items || []

    if (hienThongBao) {
      baoTin('Đã tải lại dữ liệu môn mẫu.')
    }
  } catch (error) {
    baoTin(layThongBaoLoi(error, 'Không tải được dữ liệu môn mẫu.'), 'error')
  } finally {
    dangTai.value = false
  }
}
...
async function luuMonHoc() {
  ...
  try {
    if (formMon.id) {
      ...
      baoTin('Đã cập nhật môn mẫu.')
    } else {
      await services.monHoc.create(taoPayloadMonHoc())
      baoTin('Đã thêm môn mẫu.')
    }

    await taiDuLieu({ hienThongBao: false })
    resetFormMon()
  } catch (error) {
    baoTin(layThongBaoLoi(error, 'Không lưu được môn mẫu.'), 'error')
  } finally {
    dangLuu.value = false
  }
}
...
async function xoaMonHoc(mon) {
  ...
  const dongY = window.confirm(`Xóa môn mẫu "${mon.tenMon || mon.ten || mon.maMon || mon.id}"?`)
  ...
  try {
    ...
    baoTin('Đã xóa môn mẫu.')
  } catch (error) {
    baoTin(layThongBaoLoi(error, 'Không xóa được môn mẫu. Kiểm tra môn này có đang được chương trình sử dụng không.'), 'error')
  } finally {
    dangLuu.value = false
  }
}

onMounted(() => taiDuLieu({ hienThongBao: false }))
</script>

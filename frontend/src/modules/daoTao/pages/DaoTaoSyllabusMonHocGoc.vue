          ...
          <section class="luoi-chinh">
            ...
            <div class="the khoi-danh-gia-mau">
              <div class="tieu-de-the">
                ...
                <p>Cấu hình đánh giá mẫu thuộc syllabus môn học gốc đang chọn. Khi syllabus gốc được áp dụng vào môn trong chương trình, các cột điểm mẫu này dùng để tạo/copy cấu hình đánh giá thật.</p>
              </div>
              <ThongBaoKhuVuc :state="thongBaoKhuVuc.cauHinhDanhGiaMau" />

              <div v-if="!syllabusDangChon" class="canh-bao-nho">Lưu syllabus gốc trước khi thêm cấu hình đánh giá mẫu.</div>
              <div v-else class="dong-thong-tin-danh-gia">
                <span>Đang lấy dữ liệu từ bảng <b>cau_hinh_danh_gia_mau</b></span>
                <span>Syllabus gốc ID: <b>{{ syllabusIdDangChon }}</b></span>
              ...
              <BangDonGian
                  ...
                  empty-text="Chưa có cấu hình đánh giá mẫu cho syllabus gốc này."
            ...
            <div class="the khoi-quy-doi">
              <div class="tieu-de-the">
                ...
                <p>Quy đổi điểm mẫu thuộc syllabus gốc đang chọn. Khi gán syllabus gốc vào môn trong chương trình, mẫu này dùng để tái sử dụng/copy sang bảng quy đổi điểm thật.</p>
              </div>
              <ThongBaoKhuVuc :state="thongBaoKhuVuc.quyDoiDiemMau" />

              <div v-if="!syllabusDangChon" class="canh-bao-nho">Lưu syllabus gốc trước khi thêm quy đổi điểm mẫu.</div>
              ...
              <BangDonGian
                  ...
                  empty-text="Chưa có quy đổi điểm mẫu cho syllabus gốc này."
...
const thongBaoKhuVuc = reactive({
  ...
  cauHinhDanhGiaMau: { message: 'Cấu hình đánh giá mẫu của syllabus gốc sẵn sàng.', type: 'info' },
  quyDoiDiemMau: { message: 'Quy đổi điểm mẫu của syllabus gốc sẵn sàng.', type: 'info' },
...
const formCauHinhDanhGiaMau = reactive({ id: null, syllabusMonHocGocId: '', tenCotDiem: '', loaiDiem: '', tyLe: '', diemToiDa: '', thuTu: '', batBuoc: true, ghiChu: '' })
const formQuyDoiDiemMau = reactive({ id: null, syllabusMonHocGocId: '', ma: '', ten: '', nguongTu: '', nguongDen: '', diemQuyDoi: '', ketQua: '', congThuc: '', ghiChu: '', loaiMau: 'QUY_DOI_KET_QUA', tyLe: '', diemToiDa: '', thuTu: '', batBuoc: true })
...
const cotCauHinhDanhGiaMau = [
  ...
  { key: 'syllabusMonHocGocIdText', label: 'Syllabus gốc ID', class: 'cot-syllabus-id cot-can-giua' },
  { key: 'thuTu', label: 'TT', class: 'cot-tt cot-can-giua' },
...
const danhSachCauHinhDanhGiaMauHienThi = computed(() => danhSachCauHinhDanhGiaMau.value.map((row) => ({
  ...row,
  syllabusMonHocGocIdText: hienThi(row.syllabusMonHocGocId),
...
async function taiChiTietSyllabus() {
  ...
  try {
    ...
    const [chuongBai, cauHinhDanhGiaMau, quyDoiDiemMau, ganTaiLieu, ganDieuKien] = await Promise.all([services.syllabusMonHocGocChuongBai.getAll(params), services.cauHinhDanhGiaMau.getAll(params), services.quyDoiDiemMau.getAll(params), services.syllabusMonHocGocTaiLieu.getAll(params), services.syllabusMonHocGocDieuKien.getAll(params)])
    ...
    // BE chuẩn đã filter theo syllabusMonHocGocId. Dòng dưới vẫn lọc lại ở FE để tránh hiển thị nhầm nếu API trả nhiều dữ liệu.
    rowsCauHinhDanhGiaMau = rowsCauHinhDanhGiaMau.filter((row) => String(row.syllabusMonHocGocId || '') === String(syllabusIdDangChon.value || ''))
...
function resetFormCauHinhDanhGiaMau() {
  Object.assign(formCauHinhDanhGiaMau, { id: null, syllabusMonHocGocId: syllabusIdDangChon.value || '', tenCotDiem: '', loaiDiem: '', tyLe: '', diemToiDa: '', thuTu: '', batBuoc: true, ghiChu: '' })
}
function suaCauHinhDanhGiaMau(row) {
  Object.assign(formCauHinhDanhGiaMau, {
    id: row.id,
    syllabusMonHocGocId: row.syllabusMonHocGocId || syllabusIdDangChon.value || '',
...
async function luuCauHinhDanhGiaMau() {
  if (!syllabusIdDangChon.value) return baoTin('Cần lưu syllabus gốc trước.', 'error', 'cauHinhDanhGiaMau')
...
function resetFormQuyDoiDiemMau() {
  Object.assign(formQuyDoiDiemMau, { id: null, syllabusMonHocGocId: syllabusIdDangChon.value || '', ma: '', ten: '', nguongTu: '', nguongDen: '', diemQuyDoi: '', ketQua: '', congThuc: '', ghiChu: '', loaiMau: 'QUY_DOI_KET_QUA', tyLe: '', diemToiDa: '', thuTu: '', batBuoc: true })
}
...
async function luuQuyDoiDiemMau() {
  if (!syllabusIdDangChon.value) return baoTin('Cần lưu syllabus gốc trước.', 'error', 'quyDoiDiemMau')

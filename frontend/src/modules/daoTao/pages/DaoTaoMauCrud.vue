<template>
  <div class="dao-tao-mau-mau-crud">
    ...
    <section class="the bo-loc-tang">
      ...
      <div :class="['bo-loc', { 'bo-loc-co-version': canLocVersion }]">
        ...
        <label v-if="canGanSyllabusChuongTrinhMau" class="cot-rong">
          <span>Syllabus chương trình gốc <b>*</b></span>
          <select
              v-model="boLoc.syllabusChuongTrinhMauId"
              :class="{ 'co-loi': loiForm.syllabusChuongTrinhMauId }"
              :disabled="!boLoc.chuongTrinhId || !danhSachSyllabusChuongTrinhMauTheoBoLoc.length"
              @change="xoaLoi('syllabusChuongTrinhMauId')"
          >
            <option value="">{{ !boLoc.chuongTrinhId ? 'Chọn chương trình trước' : 'Chọn syllabus chương trình gốc' }}</option>
            <option v-for="syllabus in danhSachSyllabusChuongTrinhMauTheoBoLoc" :key="syllabus.id" :value="syllabus.id">
              {{ syllabus.ma || '' }} - {{ syllabus.ten || syllabus.id }}
            </option>
          </select>
          <small v-if="loiForm.syllabusChuongTrinhMauId" class="loi-truong">{{ loiForm.syllabusChuongTrinhMauId }}</small>
          <small v-else-if="boLoc.chuongTrinhId && !danhSachSyllabusChuongTrinhMauTheoBoLoc.length" class="loi-truong">Chương trình này chưa có syllabus chương trình gốc, chưa thể CRUD {{ config.nhanDoiTuong || 'dữ liệu gốc' }}.</small>
        </label>
      </div>
    </section>
      ...
      <div class="the">
        <div class="tieu-de-the">
          <div>
            ...
            <p v-if="canGanSyllabusChuongTrinhMau"> {{ config.nhanDoiTuong || 'dữ liệu gốc' }} sẽ thuộc syllabus gốc đang chọn.</p>
            <p v-else-if="canGanChuongTrinh">Dùng Bộ lọc chương trình phía trên để chọn Ngành → Chương trình. Khi bấm thêm/cập nhật, dữ liệu sẽ thuộc chương trình đang chọn.</p>
            <p v-else>CRUD dữ liệu mẫu. Sau này vào khung chương trình chỉ chọn mẫu để copy/gán vào Version.</p>
          </div>
        </div>

        <div v-if="canGanSyllabusChuongTrinhMau" class="canh-bao-cha">
          <span v-if="boLoc.syllabusChuongTrinhMauId">{{ tenNganhDangChon }} → {{ tenChuongTrinhDangChon }} → {{ tenSyllabusChuongTrinhMauDangChon }}</span>
          <span v-else>Chưa chọn đủ Ngành → Chương trình → Syllabus chương trình gốc.</span>
        </div>

        <div v-else-if="canGanChuongTrinh" class="canh-bao-cha">
          <b>Cha hiện tại:</b>
        ...
        <form class="form-mau" @submit.prevent="luuDong">
          ...
          <div v-if="config.coTep !== false" class="cot-rong khoi-tep">
            <label>
      ...
      <div class="the">
        <div class="tieu-de-the">
          <div>
            ...
            <p>Đang hiển thị: {{ danhSachLoc.length }} / {{ danhSachmau.length }}</p>
          </div>
        </div>
        ...
        <div class="bang-boc">
          <table>
            ...
            <tr>
              ...
              <th v-if="config.coTep !== false">Tệp</th>
...
const CAU_HINH = {
  mucTieuChuongTrinhmau: {
    serviceKey: 'mucTieuChuongTrinhmau',
    joinKey: 'mucTieuChuongTrinh',
    matchBy: 'ma',
    nghiepVu: 'MUC_TIEU_CHUONG_TRINH_mau',
    tepNguon: 'mucTieuChuongTrinhmau',
    idQueryKey: 'mucTieuChuongTrinhmauId',
    tieuDeTrang: 'Mục tiêu chương trình gốc',
    nhanDoiTuong: 'mục tiêu gốc',
    moTaTrang: 'CRUD mục tiêu chương trình gốc theo Syllabus chương trình gốc. Chọn Ngành → Chương trình → Syllabus chương trình gốc; dữ liệu gốc không thuộc Version.',
    tieuDeThem: 'Thêm mục tiêu chương trình gốc',
    tieuDeCapNhat: 'Cập nhật mục tiêu chương trình gốc',
    tieuDeDanhSach: 'Danh sách mục tiêu chương trình gốc',
  ...
  nangLucDauRamau: {
    serviceKey: 'nangLucDauRamau',
    joinKey: 'chuongTrinhVersionNangLuc',
    joinIdKey: 'nangLucmauId',
    nghiepVu: 'NANG_LUC_DAU_RA_mau',
    tepNguon: 'nangLucDauRamau',
    idQueryKey: 'nangLucDauRamauId',
    tieuDeTrang: 'Năng lực đầu ra gốc',
    nhanDoiTuong: 'năng lực đầu ra gốc',
    moTaTrang: 'CRUD năng lực đầu ra gốc theo Syllabus chương trình gốc. Chọn Ngành → Chương trình → Syllabus chương trình gốc; dữ liệu gốc không thuộc Version.',
    tieuDeThem: 'Thêm năng lực đầu ra gốc',
    tieuDeCapNhat: 'Cập nhật năng lực đầu ra gốc',
    tieuDeDanhSach: 'Danh sách năng lực đầu ra gốc',
  ...
  viTriViecLammau: {
    serviceKey: 'viTriViecLammau',
    joinKey: 'chuongTrinhVersionViTriViecLam',
    joinIdKey: 'viTrimauId',
    nghiepVu: 'VI_TRI_VIEC_LAM_mau',
    tepNguon: 'viTriViecLammau',
    idQueryKey: 'viTriViecLammauId',
    tieuDeTrang: 'Vị trí việc làm gốc',
    nhanDoiTuong: 'vị trí việc làm gốc',
    moTaTrang: 'CRUD vị trí việc làm gốc theo Syllabus chương trình gốc. Chọn Ngành → Chương trình → Syllabus chương trình gốc; dữ liệu gốc không thuộc Version.',
    tieuDeThem: 'Thêm vị trí việc làm gốc',
    tieuDeCapNhat: 'Cập nhật vị trí việc làm gốc',
    tieuDeDanhSach: 'Danh sách vị trí việc làm gốc',
    nutThem: 'Thêm vị trí',
  ...
  dieuKienTotNghiepmau: {
    serviceKey: 'dieuKienTotNghiepmau',
    joinKey: 'chuongTrinhVersionDieuKienTotNghiep',
    joinIdKey: 'dieuKienmauId',
    nghiepVu: 'DIEU_KIEN_TOT_NGHIEP_mau',
    tepNguon: 'dieuKienTotNghiepmau',
    idQueryKey: 'dieuKienTotNghiepmauId',
    tieuDeTrang: 'Điều kiện tốt nghiệp gốc',
    nhanDoiTuong: 'điều kiện tốt nghiệp gốc',
    moTaTrang: 'CRUD điều kiện tốt nghiệp gốc theo Syllabus chương trình gốc. Chọn Ngành → Chương trình → Syllabus chương trình gốc; dữ liệu gốc không thuộc Version.',
    tieuDeThem: 'Thêm điều kiện tốt nghiệp gốc',
    tieuDeCapNhat: 'Cập nhật điều kiện tốt nghiệp gốc',
    tieuDeDanhSach: 'Danh sách điều kiện tốt nghiệp gốc',
    nutThem: 'Thêm điều kiện',
    nutCapNhat: 'Cập nhật điều kiện',
    coLoai: false,
    coNoiDung: true,
    coTen: false,
    coMoTa: false,
    coTep: false,
    nhanNoiDungBang: 'Nội dung điều kiện',
    dsLoai: [],
    defaultForm: { ma: '', loai: '', noiDung: '', ten: '', moTa: '', ghiChu: '', duongDan: '' }
  }
}

const config = computed(() => CAU_HINH[props.entityKey] || CAU_HINH.mucTieuChuongTrinhmau)
const canGanChuongTrinh = computed(() => [].includes(props.entityKey))
const canGanSyllabusChuongTrinhMau = computed(() => ['mucTieuChuongTrinhmau', 'nangLucDauRamau', 'viTriViecLammau', 'dieuKienTotNghiepmau'].includes(props.entityKey))
const canLocVersion = computed(() => props.entityKey === 'nhomKienThucMon')

const tieuDeBoLoc = computed(() => {
  if (canGanSyllabusChuongTrinhMau.value) return 'Bộ lọc Syllabus chương trình gốc'
  return canLocVersion.value ? 'Bộ lọc tái sử dụng' : 'Bộ lọc chương trình'
})

const moTaBoLoc = computed(() => {
  if (canGanSyllabusChuongTrinhMau.value) return 'Chọn Ngành → Chương trình → Syllabus chương trình gốc; dữ liệu gốc thuộc syllabus mẫu và không thuộc Version.'
  return canLocVersion.value
      ? 'Lọc theo Ngành → Chương trình → Version để xem dữ liệu đang liên quan đến version.'
      : 'Chọn Ngành → Chương trình để quản lý dữ liệu mẫu thuộc chương trình.'
})
...
const versionDangChon = computed(() => {
  if (!boLoc.versionId) return null
  return (duLieu.value.chuongTrinhVersion || []).find((version) => String(version.id || '') === String(boLoc.versionId || '')) || null
})

const tenVersionDangChon = computed(() => {
  const version = versionDangChon.value
  if (!version) return 'Chưa chọn version'
  return `${version.maVersion || version.ma || ''}${version.maVersion || version.ma ? ' - ' : ''}${version.tenVersion || version.ten || version.id}`
})

const syllabusChuongTrinhTheoVersionDangChon = computed(() => {
  if (!boLoc.versionId) return null
  return (duLieu.value.syllabusChuongTrinh || []).find((syllabus) => String(syllabus.chuongTrinhVersionId || '') === String(boLoc.versionId || '')) || null
})

function laySyllabusChuongTrinhMauId(syllabus) {
  return syllabus?.syllabusChuongTrinhMauId
      ?? syllabus?.syllabusChuongTrinhmauId
      ?? syllabus?.syllabusMauId
      ?? null
}

const danhSachSyllabusChuongTrinhMauTheoBoLoc = computed(() => {
  const danhSach = [...(duLieu.value.syllabusChuongTrinhmau || [])]
      .sort((a, b) => String(a.ten || a.ma || '').localeCompare(String(b.ten || b.ma || ''), 'vi'))

  if (!canGanSyllabusChuongTrinhMau.value) return danhSach
  if (!boLoc.chuongTrinhId) return []

  return danhSach.filter((item) => String(item.chuongTrinhId || '') === String(boLoc.chuongTrinhId || ''))
})

const syllabusChuongTrinhMauDangChon = computed(() => {
  if (!boLoc.syllabusChuongTrinhMauId) return null
  return (duLieu.value.syllabusChuongTrinhmau || []).find((item) => String(item.id || '') === String(boLoc.syllabusChuongTrinhMauId || '')) || null
})

const tenSyllabusChuongTrinhMauDangChon = computed(() => {
  const syllabus = syllabusChuongTrinhMauDangChon.value
  if (!syllabus) return 'Chưa chọn syllabus chương trình gốc'
  return `${syllabus.ma || ''}${syllabus.ma ? ' - ' : ''}${syllabus.ten || syllabus.id}`
})

const danhSachmau = computed(() => [...(duLieu.value[config.value.serviceKey] || [])]
...
const khoamauTheoBoLoc = computed(() => {
...
const danhSachLoc = computed(() => {
  ...
  const keysTheoTang = khoamauTheoBoLoc.value
  const cfg = config.value

  return danhSachmau.value.filter((item) => {
    if (canGanSyllabusChuongTrinhMau.value) {
      if (!boLoc.syllabusChuongTrinhMauId) return false
      if (String(item.syllabusChuongTrinhMauId || '') !== String(boLoc.syllabusChuongTrinhMauId || '')) return false
    } else if (canGanChuongTrinh.value) {
      if (boLoc.chuongTrinhId) {
...
const soCotBang = computed(() => {
  let count = config.value.coTep === false ? 3 : 4
...
async function taiDuLieu() {
  ...
  try {
    await taiDuLieuMauCrud(config.value.serviceKey)
...
function doiNganh() {
  boLoc.chuongTrinhId = ''
  boLoc.versionId = ''
  boLoc.syllabusChuongTrinhMauId = ''
}
...
function suaDong(item) {
  if (canGanSyllabusChuongTrinhMau.value && item?.syllabusChuongTrinhMauId) {
    boLoc.syllabusChuongTrinhMauId = String(item.syllabusChuongTrinhMauId)
  }
...
function validateForm() {
  ...
  if (canGanSyllabusChuongTrinhMau.value) {
    if (!boLoc.nganhId || !boLoc.chuongTrinhId) {
      loiForm.syllabusChuongTrinhMauId = 'Cần chọn đủ Ngành → Chương trình.'
      baoTin(loiForm.syllabusChuongTrinhMauId, 'error')
      return false
    }

    if (!boLoc.syllabusChuongTrinhMauId) {
      loiForm.syllabusChuongTrinhMauId = `Cần chọn syllabus chương trình gốc để gán ${config.value.nhanDoiTuong || 'dữ liệu gốc'}.`
      baoTin(loiForm.syllabusChuongTrinhMauId, 'error')
      return false
    }
  }
  ...
  const trungMa = danhSachmau.value.some((item) => {
    const cungMa = String(item.ma || '').trim().toLowerCase() === ma.toLowerCase()
    const khacDong = String(item.id || '') !== String(form.id || '')

    if (canGanSyllabusChuongTrinhMau.value) {
      const cungSyllabusMau = String(item.syllabusChuongTrinhMauId || '') === String(boLoc.syllabusChuongTrinhMauId || '')
      return cungMa && khacDong && cungSyllabusMau
    }
  ...
  if (!loiForm.ma && trungMa) {
    loiForm.ma = canGanSyllabusChuongTrinhMau.value
        ? 'Mã đã tồn tại trong syllabus chương trình gốc đang chọn.'
        : canGanChuongTrinh.value
            ? 'Mã đã tồn tại trong chương trình đang chọn.'
            : 'Mã đã tồn tại trong kho mẫu.'
  }
...
function taoPayload() {
  ...
  if (canGanSyllabusChuongTrinhMau.value) {
    payload.syllabusChuongTrinhMauId = Number(boLoc.syllabusChuongTrinhMauId)
  }
  ...
  if (cfg.coTep === false) delete payload.duongDan
...
async function luuDong() {
  ...
  try {
    ...
    if (form.id) {
      ...
      baoTin('Đã cập nhật dữ liệu mẫu.')
    } else {
      await service.create(taoPayload())
      baoTin('Đã thêm dữ liệu mẫu.')
    }
...
async function xoaDong(item) {
  if (!item?.id) return
  if (!window.confirm(`Xóa dữ liệu mẫu "${item.ma || item.ten || item.id}"?`)) return
...
watch(
    () => boLoc.chuongTrinhId,
    () => {
      if (!canGanSyllabusChuongTrinhMau.value) return
      const danhSach = danhSachSyllabusChuongTrinhMauTheoBoLoc.value
      const dangChonConHopLe = danhSach.some((item) => String(item.id || '') === String(boLoc.syllabusChuongTrinhMauId || ''))
      if (!dangChonConHopLe) boLoc.syllabusChuongTrinhMauId = ''
      resetForm()
    }
)

  ...
  <section class="schedule-page">
    <header class="page-head">
      <div>
        ...
        <h1>Xem lịch dạy</h1>
        <p>Theo dõi lớp/môn, phòng, ca dạy, thứ trong tuần và ngày trong tháng.</p>
      </div>
    ...
    <section class="summary-grid">
      <div><span>Số buổi dạy</span><strong>{{ danhSach.length }}</strong></div>
      <div><span>Số ngày có lịch</span><strong>{{ soNgayCoLich }}</strong></div>
      <div><span>Quy tắc ca</span><strong>2 ca liên tục</strong></div>
    </section>
    ...
    <section class="matrix-card">
      <div class="matrix-head">
        <div>
          <h2>Ma trận lịch dạy tuần</h2>
          <p>Những thẻ liền nhau theo ca thể hiện lịch dạy liên tục trong cùng ngày.</p>
        </div>
      </div>
        ...
        <div class="matrix-grid" :style="matrixGridStyle">
          ...
          <template v-for="ca in danhSachCaHoc" :key="ca.id">
            ...
            <div v-for="ngay in danhSachNgayTrongTuan" :key="`${ca.id}-${ngay.iso}`" class="matrix-cell">
              ...
              >
                <div class="card-title">{{ item.tenLop || item.maLop || 'Lớp học' }}</div>
    ...
    <section class="list-card">
      <h2>Lịch dạy dạng danh sách</h2>
      <div v-if="dangTai" class="empty-list">Đang tải lịch dạy...</div>
      <div v-else-if="!danhSach.length" class="empty-list">Chưa có lịch dạy trong tuần này.</div>
      <div v-else class="day-list">
        <article v-for="item in danhSachSapXep" :key="item.id" class="list-item">
          ...
          <span>{{ item.tenLop || item.maLop || '-' }} | Phòng: {{ item.tenPhong || item.maPhong || '-' }} | {{ hienThiTrangThai(item.trangThai) }}</span>
        </article>
      </div>
    </section>
  </section>
...
const soNgayCoLich = computed(() => new Set(danhSach.value.map((item) => item.ngayHoc).filter(Boolean)).size)
...
async function taiDuLieu() {
  ...
  loi.value = ''
  try {
    const page = await giangDayService.layLichDayGiangVien({
      page: 0,
      size: 1000,
      tuNgay: boLoc.tuNgay,
    ...
    loi.value = error?.message || 'Không tải được lịch dạy giảng viên'
  } finally {
    dangTai.value = false
  }
}
...
function doiTuan(delta) { boLoc.tuNgay = congNgayIso(boLoc.tuNgay, delta * 7); boLoc.denNgay = congNgayIso(boLoc.tuNgay, 6); taiDuLieu() }
function veTuanHienTai() { boLoc.tuNgay = dauTuanIso(new Date()); boLoc.denNgay = congNgayIso(boLoc.tuNgay, 6); taiDuLieu() }
...
.toolbar-card, .matrix-card, .list-card, .summary-grid > div { padding: 16px; border: 1px solid var(--color-border, #e2e8f0); background: #fff; border-radius: var(--radius, 14px); }
.summary-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
.summary-grid div { display: grid; gap: 6px; }
.summary-grid span { color: #64748b; font-weight: 800; font-size: 13px; }
...
@media (max-width: 760px) { .page-head, .summary-grid { grid-template-columns: 1fr; flex-direction: column; } }
</style>

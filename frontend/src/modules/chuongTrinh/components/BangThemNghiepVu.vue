  ...
  <section :class="['bang-them-nghiep-vu', `bang-${loaiBang}`, { disabled, collapsed: !moBang }]">
      ...
      <template v-for="field in visibleFields" :key="field.key">
        <label class="field" :class="[field.wide ? 'wide' : '', fieldErrors[field.key] ? 'has-error' : '']">
          ...
          <input
              ...
              :max="field.type === 'number' ? (field.max ?? 999999.9) : undefined"
              :step="field.type === 'number' ? (field.step ?? (field.key === 'thuTu' ? 1 : 0.1)) : undefined"
              @input="validateField(field)"
          />
    ...
    <div class="table-wrap mini-scroll">
      <table>
        <thead>
        <tr>
          <th class="col-action col-action-wide">Chọn / Xem / Lưu</th>
        ...
        <template v-else>
          ...
          >
            <td class="col-action col-action-wide action-pair">
              ...
              <button type="button" :class="['btn tiny view-btn', { viewing: isViewing(item), related: isRelated(item) }]" title="Xem dữ liệu con" @click="viewItem(item)">Xem</button>
              <button
                  v-if="canToggleSave"
                  type="button"
                  :class="['btn tiny save-link-btn', { saved: isSaved(item) }]"
                  :title="isSaved(item) ? 'Bỏ lưu khỏi luồng đang chọn' : 'Lưu vào luồng đang chọn'"
                  @click="toggleSaveItem(item)"
              >
                {{ isSaved(item) ? 'Đã lưu' : 'Lưu' }}
              </button>
            </td>
...
const props = defineProps({
  ...
  loaiBang: {
    type: String,
    default: 'phu',
    validator: (value) => ['xuong-song', 'phu'].includes(value)
  },
...
function layCacDongKiemTraTrung() {
  return Array.isArray(props.allItems) && props.allItems.length ? props.allItems : props.items
}
...
function validateForm() {
  ...
  if (!ok) {
    ...
    emit('notify', firstError || 'Vui lòng kiểm tra lại các ô đang báo lỗi.', 'error')
  }
